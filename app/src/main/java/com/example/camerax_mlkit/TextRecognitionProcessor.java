...
import com.example.camerax_mlkit.VisionProcessorBase;
...
public class TextRecognitionProcessor extends VisionProcessorBase<Text> {
  ...
  //文字認識が成功した時に実行。TextGraphicに取得したテキストを渡す
  @Override
  protected void onSuccess(@NonNull Text text, @NonNull GraphicOverlay graphicOverlay) {

    String deteText = text.getText();
    if(!deteText.equals("")) {
      Intent intent = new Intent(graphicOverlay.getContext(), SubActivity.class);
      intent.putExtra("text", deteText);
