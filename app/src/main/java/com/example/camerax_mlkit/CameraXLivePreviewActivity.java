...
import com.example.camerax_mlkit.CameraXViewModel;
import com.example.camerax_mlkit.GraphicOverlay;

import com.example.camerax_mlkit.VisionImageProcessor;
import com.example.camerax_mlkit.TextRecognitionProcessor;
import com.example.camerax_mlkit.PreferenceUtils;
import com.google.mlkit.vision.text.japanese.JapaneseTextRecognizerOptions;
...
//  private static final String FACE_MESH_DETECTION = "Face Mesh Detection (Beta)";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // onCreate() メソッド内で、アクティビティの前回の状態が保存されている場合に、
    // 特定の変数の値を復元する
...
//  }
  ...
  private void bindAllCameraUseCases() {
    if (cameraProvider != null) {
      //  CameraX APIの要件に従い、再バインドする前にすべてのユースケースをアンバインドします。
  ...
  private void bindPreviewUseCase() {
    System.out.println("testbind"+PreferenceUtils.isCameraLiveViewportEnabled(this));
    if (!PreferenceUtils.isCameraLiveViewportEnabled(this)) {
      System.out.println("PreferenceUtilstest");
      return;
    }
    if (cameraProvider == null) {
      return;
    ...
    //カメラのライフサイクルにバインドされ、プレビューが開始される。
    camera =
